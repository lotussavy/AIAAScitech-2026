...
Use Python 3.12 and install `requirements-notebooks.txt`. Run `python -m unittest discover -s tests -v` and `python -m uam_demand --verify-inputs` before proposing changes. Run `python scripts/execute_notebooks.py` when changing notebook interfaces.
